...
useSearch reruns fuzzy search whenever refreshToken changes, and that token changes after uploads, deletes, and pending-status updates. So a user can click AI search, see reranked results, and then lose them as soon as background polling refreshes the gallery. Simplest fix: stop coupling search invalidation to gallery refresh, and make the active search mode/request authoritative with cancellation or request IDs.~~
   ...
   **Fixed:** Removed `refreshToken` from `useSearch`'s parameter and `useEffect` dependency array. The fuzzy search now only reruns when `searchQuery` or `hasSearchQuery` changes — not on gallery refreshes. `App.jsx` updated to call `useSearch()` with no arguments. LLM results set via `runAiSearch` are now stable until the user changes the query.
...
recent_statuses makes the endpoint return short-lived done/error items that are not coming from the DB query, which means behavior depends on the current worker and disappears on restart. That is fine for a toy single-process app, but it is a poor API contract because the endpoint is no longer authoritative. Simplest fix: keep the job/status truth in SQLite and return only DB-backed states.~~
   ...
   **Fixed:** Deleted `recent_statuses`, `remember_status`, `recent_status_snapshot`, and `RECENT_STATUS_WINDOW_SECONDS` from `main.py`. `pending_statuses()` in `repository.py` now queries `analysis_status IN ('pending', 'error')` so error items remain visible via the DB. The `/api/memes/pending` endpoint returns `repo.pending_statuses()` directly. The polling hook's change-detection and stop condition still work correctly: a transition from pending to done removes the item from the response (snapshot changes → gallery refresh); error items persist in the response as a DB-backed terminal state.
...
init_db() drops and recreates triggers, deletes all FTS rows, and reinserts every analyzed meme on each startup. Since the triggers already maintain the index during normal operation, this turns app boot into a full-table write pass for no steady-state benefit. Simplest fix: separate one-time schema setup/migrations from runtime startup, and only rebuild FTS when schema/version changes require it.~~
   ...
   **Fixed:** Removed `DROP TABLE IF EXISTS memes_fts`, `DROP TRIGGER IF EXISTS` for all three triggers, `DELETE FROM memes_fts`, and the full reinsert block from `init_db.py`. All `CREATE VIRTUAL TABLE` and `CREATE TRIGGER` statements now use `IF NOT EXISTS`, making `init_db()` fully idempotent. The triggers already keep the FTS index in sync during normal operation, so no rebuild is needed on startup.
